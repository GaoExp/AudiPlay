# Struktur Projectfolder
26 folder
105 file

```
AudiPlay/
│
├── AGENTS.md
├── CHANGELOG.md
├── PANDUAN.md
├── README.md
├── STRUKTUR.md
├── build.gradle
├── gradle.properties
├── gradlew
├── gradlew.bat
├── release.sh
├── settings.gradle
│
├── gradle/
│
└── app/
    ├── build.gradle
    ├── proguard-rules.pro
    ├── libs/
    ├── schemas/
    │
    └── src/main/
        ├── AndroidManifest.xml
        │
        ├── assets/
        │   ├── CHANGELOG.txt
        │   ├── PANDUAN.txt
        │   ├── README.txt
        │   └── STRUKTUR.txt
        │
        ├── java/exp/miniplayer/
        │   ├── MainActivity.java
        │   ├── MiniPlayerApp.java
        │   │
        │   ├── adapter/
        │   │   ├── PlaylistAdapter.java
        │   │   ├── PlaylistDetailAdapter.java
        │   │   └── SongAdapter.java
        │   │
        │   ├── data/
        │   │   └── AudioRepository.java
        │   │
        │   ├── database/
        │   │   ├── AppDatabase.java
        │   │   ├── FavoriteDao.java
        │   │   ├── FavoriteEntity.java
        │   │   ├── PlaylistDao.java
        │   │   ├── PlaylistEntity.java
        │   │   ├── PlaylistSongDao.java
        │   │   └── PlaylistSongEntity.java
        │   │
        │   ├── model/
        │   │   └── Audio.java
        │   │
        │   ├── player/
        │   │   └── MusicPlayer.java
        │   │
        │   ├── service/
        │   │   └── MusicService.java
        │   │
        │   ├── ui/
        │   │   ├── documentation/
        │   │   │   └── DocumentationActivity.java
        │   │   ├── favorites/
        │   │   │   ├── FavoritesFragment.java
        │   │   │   └── FavoritesViewModel.java
        │   │   ├── playlist/
        │   │   │   ├── PlaylistDetailActivity.java
        │   │   │   ├── PlaylistDetailViewModel.java
        │   │   │   ├── PlaylistFragment.java
        │   │   │   └── PlaylistViewModel.java
        │   │   ├── settings/
        │   │   │   ├── SettingsFragment.java
        │   │   │   └── SettingsViewModel.java
        │   │   └── songs/
        │   │       ├── SongsFragment.java
        │   │       └── SongsViewModel.java
        │   │
        │   └── utils/
        │       ├── MusicScanner.java
        │       ├── PermissionHelper.java
        │       ├── PreferencesManager.java
        │       ├── QueueHolder.java
        │       └── TimeUtils.java
        │
        └── res/
            ├── anim/
            │   ├── slide_in_up.xml
            │   ├── slide_out_down.xml
            │   └── stay.xml
            │
            ├── drawable/
            │   ├── drag_handle_background.xml
            │   ├── ic_album_default.xml
            │   ├── ic_arrow_down.xml
            │   ├── ic_bg_purple.xml
            │   ├── ic_bg_surface.xml
            │   ├── ic_close.xml
            │   ├── ic_favorite.xml
            │   ├── ic_favorite_border.xml
            │   ├── ic_fg_note.xml
            │   ├── ic_fg_note_dark.xml
            │   ├── ic_fg_play.xml
            │   ├── ic_fg_play_dark.xml
            │   ├── ic_fg_queue.xml
            │   ├── ic_launcher_background.xml
            │   ├── ic_launcher_foreground.xml
            │   ├── ic_music_note.xml
            │   ├── ic_overflow.xml
            │   ├── ic_pause.xml
            │   ├── ic_play.xml
            │   ├── ic_playlist_add.xml
            │   ├── ic_queue_music.xml
            │   ├── ic_repeat.xml
            │   ├── ic_repeat_one.xml
            │   ├── ic_search.xml
            │   ├── ic_settings.xml
            │   ├── ic_shuffle.xml
            │   ├── ic_skip_next.xml
            │   └── ic_skip_previous.xml
            │
            ├── layout/
            │   ├── activity_documentation.xml
            │   ├── activity_main.xml
            │   ├── activity_playlist_detail.xml
            │   ├── dialog_audio_formats.xml
            │   ├── dialog_folder_input.xml
            │   ├── dialog_folder_list.xml
            │   ├── fragment_favorites.xml
            │   ├── fragment_playlist.xml
            │   ├── fragment_settings.xml
            │   ├── fragment_songs.xml
            │   ├── item_folder.xml
            │   ├── item_playlist.xml
            │   ├── item_song.xml
            │   └── view_now_playing_sheet.xml
            │
            ├── menu/
            │   ├── bottom_nav_menu.xml
            │   ├── now_playing_options_menu.xml
            │   ├── song_options_menu.xml
            │   └── songs_menu.xml
            │
            ├── mipmap-anydpi-v26/
            │   ├── ic_launcher.xml
            │   ├── ic_launcher_note.xml
            │   ├── ic_launcher_note_dark.xml
            │   ├── ic_launcher_note_dark_round.xml
            │   ├── ic_launcher_note_round.xml
            │   ├── ic_launcher_play.xml
            │   ├── ic_launcher_play_dark.xml
            │   ├── ic_launcher_play_dark_round.xml
            │   ├── ic_launcher_play_round.xml
            │   ├── ic_launcher_queue.xml
            │   ├── ic_launcher_queue_round.xml
            │   └── ic_launcher_round.xml
            │
            ├── values/
            │   ├── colors.xml
            │   ├── dimens.xml
            │   ├── strings.xml
            │   └── themes.xml
            │
            └── xml/
                └── backup_rules.xml
```
---
