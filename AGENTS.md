# AGENTS.md — Aturan AI Project AudiPlay (JAVA)

> **Penting:** Baca ulang file ini setiap mulai bekerja. JANGAN commit/tag/push tanpa perintah user. Eksekusi hanya setelah perintah eksplisit. Tindakan destruktif WAJIB konfirmasi. Build **hanya dijalankan saat diperintah langsung** oleh user — jangan meminta izin/konfirmasi untuk build, cukup diam.
>
...
- **Changelog = informasi perubahan untuk user, bukan catatan riwayat kerja agent/developer.** Perubahan yang tidak mengubah perilaku yang dirasakan user — prototipe/eksplorasi yang belum final, sarana/tool internal developer (mis. panel Debugging) — tidak dicatat sebagai poin.
