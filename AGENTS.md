...
removed +1, minor +1, patch reset

Fitur kembali:

`1.2.2.18.0`

restored +1, minor +1, patch reset
...
Feature removed:
removed+1, minor+1, patch=0

Feature restored:
restored+1, minor+1, patch=0
...
- `app/src/main/assets/CHANGELOG.md` — in-app, sinkron otomatis via Gradle task `syncDocs` saat build
...
### Sinkronisasi file .md

`syncDocs` (Gradle task `preBuild`) otomatis menyalin file .md dari root ke `app/src/main/assets/`:
- `README.md` / `PANDUAN.md` / `STRUKTUR.md` / `CHANGELOG.md`

Setelah mengubah entry CHANGELOG, jalankan `cp CHANGELOG.md app/src/main/assets/` jika tidak build, agar assets tetap sinkron.
...
4. jika tidak build, `cp *.md app/src/main/assets/` agar assets sinkron
...
Penerapan berlaku kepada semua Agent AI yang terlibat dalam pengerjaan Project.
...
Utamakan:
- baca agents.md setiap kali memulai pekerjaan
- periksa status commit/log apakah sudah di push
- perubahan minimal dan terfokus
- jawaban singkat, langsung bisa dipakai

### Chat Response Rules

Default response style:
- singkat, fokus, actionable
- tulis ulang percakapan terakhir di `/temporary/CHAT` jika chat terpotong/terputus (entry baru di atas)
