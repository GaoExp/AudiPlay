# [1.1.0.12.0] - 2026-06-14

### ✨ Fitur Baru
- **Dokumentasi Markdown** — Penampil dokumentasi kini render `.md` dengan Markwon (tabel, tasklist). Tidak lagi plain text.
- **Zoom di toolbar** — Kontrol ukuran teks (−/+) di toolbar, bukan bar terpisah.

### ♻️ Perubahan Fitur
- `app_name` dari "Mini Player" → "AudiPlay"
- `apply plugin` diganti dengan `plugins { id 'com.android.application' }`
- APK output otomatis rename: `AudiPlay-v{versionCode}.apk`

### 🔧 Optimasi & Penyesuaian
- AGENTS.md — format & konten dirapikan, tambah aturan sinkronisasi assets, hapus kontradiksi
- gradle.properties — dirapikan dengan komentar, tambah opsi daemon/paralel, set console=auto
- CHANGELOG.txt (assets) — bersihkan conflict markers (`<<<<<<< HEAD`) yang tersisa
- `release.yml` — perbaiki AWK pattern heading level-1
- Dokumentasi asset dari `.txt` → `.md`

### 🗒️ File Added
- `app/src/main/res/layout/toolbar_zoom.xml` — Kontrol zoom di toolbar
- `app/src/main/assets/CHANGELOG.md`, `README.md`, `PANDUAN.md`

### ✏️ File Changed
- `AGENTS.md` — rapi format, tambah aturan baru
- `app/build.gradle` — plugins block, APK rename, Markwon dependencies
- `gradle.properties` — tambah komentar & opsi build
- `app/src/main/res/values/strings.xml` — ubah app_name
- `app/src/main/java/exp/miniplayer/ui/documentation/DocumentationActivity.java` — rewrite: Markwon, MD files, zoom toolbar
- `app/src/main/res/layout/activity_documentation.xml` — rewrite: MaterialCardView list + Markdown content
- `.github/workflows/release.yml` — ganti secret names, AWK fix
- `README.md` — update versi ke 1.1.0.12.0

### 🔥 File Removed
- `app/src/main/assets/CHANGELOG.txt`, `README.txt`, `PANDUAN.txt`

### 🔢 Version
versionCode: 31
versionName: 1.1.0.12.0
